...
def parser_function():
    with open(filepath) as file:
        ...
        for i in file: 
            ...
            # dlc = line[3]
            payload_str = line[4:] #grab the payload
            payload_int = [int(x,16) for x in payload_str] #make it a list of ints
            payload_byte = bytes(payload_int) #convert each int to a byte
            ...
            msg = can.Message(
                ...
                data=payload_byte,
                is_extended_id=True #true because 29 bit IDs
            )
            # print(f"sending: {msg}")
            bus.send(msg)
            read_can()
